...
class CandleManager:
    ...
    def __init__(self, api: OandaApi, trade_settings, log_message, granularity):
        ...
        self.timings = { pair: CandleTiming(self.api.last_complete_candle(pair, self.granularity)) for pair in self.pairs_list }
    ...
    def update_timings(self):
        ...
        for pair in self.pairs_list:
            ...
            if current > self.timings[pair].last_time:
                ...
                self.log_message(f"CandleManager() init new candle:{self.timings[pair]}", pair)
