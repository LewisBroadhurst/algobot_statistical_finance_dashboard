...
def load_data(pair, time_d=1):
    ...
    start = parser.parse("2020-01-01T00:00:00Z")
...
def run_process(pair):
    print("PROCESS STARTED", pair)
    results = run_pair(pair)
    results.to_pickle(f"./exploration/macd_ema/macd_ema_res_{pair}.pkl")
    print("PROCESS ENDED", pair)


def get_sim_pairs(ic: InstrumentCollection):
    pairs = []
