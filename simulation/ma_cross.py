...
class MAResult:
    ...
    def __init__(self, df_trades, pairname, ma_s, ma_l, granularity):
...
def run_ma_sim(curr_list = LMBROADHURST_PAIRS, granularity = ["H1", "H4"], ma_short = [10, 20], ma_long = [20, 40, 80], filepath = "./data"):

    ic.LoadInstruments("./data")
    for g in granularity:
        for pair in curr_list:
            if pair in ic.instruments_dict.keys():
                analyse_pair(ic.instruments_dict[pair], g, ma_short, ma_long, filepath)
...
def get_trades(df_analysis, instrument, granularity):
    ...
    df_trades['GAIN'] = (df_trades.DIFF / instrument.pipLocation) * df_trades['TRADE']
...
def assess_pair(price_data, ma_s, ma_l, instrument, granularity):
    df_analysis = price_data.copy()
    df_analysis["DELTA"] = df_analysis[ma_l] - df_analysis[ma_s]
...
def process_results(results_list, filepath):
    process_macro(results_list, get_fullname(filepath, "ma_results"))
    process_trades(results_list, get_fullname(filepath, "ma_trades"))

    
def analyse_pair(instrument, granularity, ma_short, ma_long, filepath):
    ...
    for ma_l in ma_long:
        for ma_s in ma_short:
            ...
            ma_result = assess_pair(price_data, get_ma_col(ma_s), get_ma_col(ma_l), instrument, granularity)
